...
#define CDD_IPC_NUM_OF_BUFFERS            (1U)

/** \brief Size of each buffer for the RpMsg*/
#define CDD_IPC_MAX_BUFFER_SIZE              (1152U)
...
#define CDD_IPC_SHARED_MEMORY_SIZE (7296U)


#define CDD_IPC_R5FSS0_0_ID   (0U)
#define CDD_IPC_R5FSS0_1_ID   (1U) 
#define CDD_IPC_C66x_DSP_ID   (2U) 
...
#define CDD_IPC_DEINIT_API                                        (STD_OFF)
...
#define CDD_IPC_RPMSG_CALLBACK_ENABLE                             (STD_ON)
/** \brief CDD Rpmsg Recv NOTIFY Callback Enable Switch */
#define CDD_IPC_RPMSG_NOTIFY_CALLBACK_ENABLE                      (STD_OFF)

#define CDD_IPC_SAFEIPC_ENABLE          (STD_OFF)

#define CDD_IPC_CRC_ENABLE              (STD_OFF)
...
typedef struct
{
    uint32 Cdd_Ipc_localEndPt;  /**< local end point at which to listen for received mesages */
    Cdd_Ipc_RPMessage_lld_recvCallback recvCallback; /**< Optional callback to invoke when a message is received at this end point */
    void *recvCallbackArgs;  /**< Arguments to pass to the `recvCallback` callback */
    Cdd_Ipc_RPMessage_lld_recvNotifyCallback recvNotifyCallback; /**< Optional callback to notify user when a message is received at this end point. If `recvCallback` is set, then this callback is not used */
...
typedef struct
{
    uint32 Cdd_Ipc_SelfCoreId; /* core ID on which this module is running */
    uint32 Cdd_Ipc_numCores;   /**< Number of remote cores participating in IPC, excluding the core on
                               *  which this API is called.*/
    uint32 Cdd_Ipc_coreIdList[CDD_IPC_CORE_ID_MAX]; /**< List of Core ID's participating in IPC, excluding the core on
                                                    *   which this API is called.*/
    #if(CDD_IPC_RPMSG_ENABLE_API == STD_ON)
    Cdd_IpcCreateParams Cdd_Ipc_RpMsgParams[CDD_IPC_LOCALEP_COUNT]; /* Params structure for holding the local end point ID */
    #endif
...
/** \brief Pointer to the Cdd Ipc Configuration struct declaration for usage in driver*/
