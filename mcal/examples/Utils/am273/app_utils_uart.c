...
#define UART_BUFF_SIZE 512U
static uint8 logUartBuff[UART_BUFF_SIZE];

void Enable_Uart(uint32 Clk_freq)
{
    uint32 baudRate = 0U;
    ...
    /* Setup the logger SCI baud rate */
    /*
     * Set baud-rate, Compute Baud value = (Sys_Freq/(16*(E_SCI_BAUD_115200))) - 1
     * Sys_Freq*M_SCI_BAUD_CONST_115200_S18 - 1
     */
    baudRate = ((((uint32)Clk_freq * (uint32)M_SCI_BAUD_CONST_115200_S18) + ((uint32)1U << 17U)) >> 18U) - (uint32)1U;
    sciSetBaudrateValue(sciAREG, baudRate);
}
...
void UARTPrintf(const char *pcFormat, ...)
{
    ...
    while (TRUE)
    {
        ...
        va_end(list);
        if ((iRet > -1) && ((uint32)iRet < UART_BUFF_SIZE))
        {
            break;
        }
        else
