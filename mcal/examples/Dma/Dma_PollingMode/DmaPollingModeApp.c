...
#include "soc.h"
#include "DmaPollingModeApp.h"
#include "DmaPollingModeApp_Startup.h"
#include "CacheP.h"
#include "Cdd_Dma.h"
...
uint32              handleId = CddDmaConf_CddDmaDriverHandler_MemoryTransfer_Polling;
...
static void Cdd_Dma_PollingModeApp_mainTest(void)
{
    AppUtils_TimerInit();
    uint8             *srcPtr, *dstPtr;
    ...
    for (uint32 count = 0; count < (CDD_DMA_TEST_B_COUNT * CDD_DMA_TEST_C_COUNT); count++)
    {
        ...
        /* Checking status in loop and will come out only after transfer completion */
        while (Cdd_Dma_GetStatus(handleId) == 0)
            ;
    }
...
int main(void)
{
    Cdd_Dma_PollingModeApp_Startup();
