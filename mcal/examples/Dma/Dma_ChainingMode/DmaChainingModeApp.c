...
#include "soc.h"
#include "DmaChainingModeApp.h"
#include "DmaChainingModeApp_Startup.h"
#include "CacheP.h"
#include "Cdd_Dma.h"
...
uint32              handleId = CddDmaConf_CddDmaDriverHandler_MemoryTransfer_Chaining;
...
/* Callback registered for the Chaining */
...
static void Cdd_Dma_ChainingModeApp_mainTest(void)
{
    AppUtils_TimerInit();
    uint8             *srcPtr, *dstPtr;
    Cdd_Dma_ParamEntry paramEntry0, paramEntry1;
    uint32             opt0, opt1, chainOption, shift, status = MCAL_SystemP_SUCCESS;
    ...
    /* Address shift for te next channel transfer */
    ...
    /* Checking to validate that transfer happen successfully */
    for (uint32 count = 0; count < CDD_DMA_TEST_BUFFER_SIZE; count++)
    {
...
int main(void)
{
    Cdd_Dma_ChainingModeApp_Startup();
    ...
    AppUtils_printf("DmaChainingModeApp: Sample Application - STARTS !!!\n\r ");

    Cdd_Dma_ChainingModeApp_mainTest();
    return 0;
}
