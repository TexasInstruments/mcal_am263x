 ...
 * @{
 */
#define PWM_PRE_COMPILE_VARIANT         ([!IF "as:modconf('Pwm')[1]/IMPLEMENTATION_CONFIG_VARIANT = 'VariantPreCompile'"!]STD_ON [!ELSE!]STD_OFF[!ENDIF!])

#define PWM_VARIANT_POST_BUILD          ([!IF "as:modconf('Pwm')[1]/IMPLEMENTATION_CONFIG_VARIANT = 'VariantPostBuild'"!]STD_ON [!ELSE!] STD_OFF[!ENDIF!])
...
#define PWM_CFG_MAJOR_VERSION           ([!"substring-before($moduleSoftwareVer,'.')"!]U)
/** \brief Pwm configuration Minor Version */
#define PWM_CFG_MINOR_VERSION           ([!"substring-before(substring-after($moduleSoftwareVer,'.'),'.')"!]U)
/** \brief Pwm configuration Patch Version */
#define PWM_CFG_PATCH_VERSION           ([!"substring-after(substring-after($moduleSoftwareVer,'.'),'.')"!]U)
/* @} */
...
#define PWM_DEINIT_API      ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmDeInitApi"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM GetOutputState API */
#define PWM_GET_OUTPUT_STATE_API		([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmGetOutputState"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM SetDutyCycle API */
#define PWM_SET_DUTY_CYCLE_API      ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmSetDutyCycle"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM SetOutputToIdle API */
#define PWM_SET_OUTPUT_TO_IDLE_API      ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmSetOutputToIdle"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM SetPeriodAndDuty API */
#define PWM_SET_PERIOD_AND_DUTY_API     ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmSetPeriodAndDuty"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM VersionInfo API */
#define PWM_VERSION_INFO_API    ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmVersionInfoApi"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM RegisterReadback API */
#define PWM_REGISTER_READBACK_API       ([!IF "as:modconf('Pwm')[1]/PwmConfigurationOfOptApiServices/PwmEnableRegisterReadbackApi"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM Dutycycle update always at the end of the period */
#define PWM_DUTYCYCLE_UPDATED_ENDPERIOD     ([!IF "as:modconf('Pwm')[1]/PwmGeneral/PwmDutycycleUpdatedEndperiod"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM Period update always at the end of the period */
#define PWM_PERIOD_UPDATED_ENDPERIOD    ([!IF "as:modconf('Pwm')[1]/PwmGeneral/PwmPeriodUpdatedEndperiod"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM dev detect error */
#define PWM_DEV_ERROR_DETECT              ([!IF "as:modconf('Pwm')[1]/PwmGeneral/PwmDevErrorDetect"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM notifications */
#define PWM_NOTIFICATION_SUPPORTED        ([!IF "as:modconf('Pwm')[1]/PwmGeneral/PwmNotificationSupported"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])

/** \brief Enable/Disable PWM SFO Support for HRPWM */
#define PWM_SFO_SUPPORT_ENABLE             ([!IF "as:modconf('Pwm')[1]/PwmGeneral/PwmSfoSupportEnable"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!]) 
/* @} */
/** \brief Instance ID for driver module. */
#define PWM_INDEX       ([!"as:modconf('Pwm')[1]/PwmGeneral/PwmIndex"!]U)
#define PWM_INSTANCE_ID                 (PWM_INDEX)

/** \brief ISR type */
#define PWM_ISR_TYPE    ([!"as:modconf('Pwm')[1]/PwmGeneral/PwmTypeofInterruptFunction"!])
[!AUTOSPACING!]
[!VAR "var1" = "0"!]
[!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet"!][!VAR "var1" = "0"!][!LOOP "PwmChannel/*"!]
[!VAR "var1" = "$var1+1"!][!ENDLOOP!][!ENDLOOP!]

/** \brief Number of configured channels */
#define PWM_MAX_NUM_CHANNELS   ([!"num:i($var1)"!]U)
...
#define PwmConf_PwmChannel_[!"@name"!] [!WS "8"!] ([!"PwmChannelId"!]U)
/**< Channel identifiers */
[!ENDLOOP!][!//
...
#define PWM_INIT_CONFIG_PC       [!"@name"!]
[!ENDLOOP!]
[!ENDIF!]

[!VAR "Index5" = "0"!]
[!LOOP "as:modconf('Pwm')[1]/PwmChannelConfigSet/PwmChannel/*"!]
/** \brief EPWM Channel [!"num:i(PwmChannelId + 1)"!], in MAIN DOMAIN */
#define PWM_CHANNEL_[!"PwmHwUnit"!]_PWMCHANNEL[!"num:i(PwmHWChannelId)"!]   ([!"PwmHWChannelId"!])
[!VAR "Index5" = "$Index5+1"!]
[!ENDLOOP!][!//

/** \brief OS counter ID - used for timeout in case of error */
#define PWM_OS_COUNTER_ID           ([!IF "node:refexists(as:modconf('Pwm')[1]/PwmGeneral/PwmOsCounterRef)"!](CounterType)[!"node:name(node:ref(as:modconf('Pwm')[1]/PwmGeneral/PwmOsCounterRef))"!][!ELSE!][!"as:modconf('Pwm')[1]/PwmGeneral/PwmDefaultOSCounterId"!]U[!ENDIF!])
 ...
 *   below value
 */
#define PWM_MAX_TIMEOUT_DURATION        ([!"as:modconf('Pwm')[1]/PwmGeneral/PwmMaxTimeoutDuration"!]U)
...
extern const struct Pwm_ConfigType_PC_s [!"@name"!]_PC;
extern const struct Pwm_ConfigType_s [!"@name"!];
[!ENDLOOP!]
